...
`kt-defaults` 是官方默认智能体包，包含开箱即用的 creatures（`swe`、`reviewer`、`researcher`、`ops`、`creative`、`general`、`root`）以及几个 terrariums。
...
安装完成后，包会存放在 `~/.kohakuterrarium/packages/kt-defaults/`，你可以用 `@kt-defaults/path` 语法引用里面的内容。可用资源如下：

| creature | 用途 |
|----------|------|
| `@kt-defaults/creatures/general` | 通用助手 |
| `@kt-defaults/creatures/swe` | 软件工程助手 |
| `@kt-defaults/creatures/reviewer` | 代码审查助手 |
| `@kt-defaults/terrariums/swe_team` | 多智能体协作示例 |
