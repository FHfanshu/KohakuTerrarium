...
这篇对应的示例代码在：`examples/agent-apps/swe_bio_agent`。这是一个带 `kohaku.yaml` 的本地 package，实际 creature 在 `examples/agent-apps/swe_bio_agent/creatures/swe_bio_agent`。
...
examples/agent-apps/swe_bio_agent/
  kohaku.yaml                         # package 入口
  creatures/
    swe_bio_agent/
      config.yaml                     # creature 主配置
      prompts/
        system.md                     # 系统提示词
      custom/
        ...
        doc_reviewer.py               # 文档 AI 痕迹检查子智能体
      artifacts/
        audit/                        # 日志输出目录
```
...
prompt 只能起到“提醒”作用，模型不一定总会照做。真正负责硬拦截的是 `creatures/swe_bio_agent/custom/guard_plugin.py`：
...
`creatures/swe_bio_agent/custom/audit_plugin.py` 会把这些信息写进 JSONL：
...
# 看审计日志
dir examples/agent-apps/swe_bio_agent/creatures/swe_bio_agent/artifacts/audit/

# 看日志内容
Get-Content examples/agent-apps/swe_bio_agent/creatures/swe_bio_agent/artifacts/audit/*.jsonl | ConvertFrom-Json
```
