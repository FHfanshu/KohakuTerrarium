...
这些通用 trigger 类都会通过 `modules/trigger/callable.py:CallableTriggerTool` 包一层，变成单独的工具。要启用的话，把 trigger 的 `setup_tool_name` 写进 `tools:`，再指定 `type: trigger`。工具描述前面会带 `**Trigger** — `，提醒 LLM 这不是一次性调用，而是会装一个长期生效的东西。调用后会立刻返回 trigger id，真正的 trigger 在后台跑。
