...
`Conversation` 用来管消息历史，也支持序列化成 OpenAI 那套格式。
...
模块：`kohakuterrarium.serving.manager`。这是个不绑传输层的管理器；HTTP API 和自定义传输都会用到它。
...
- `async agent(config_path: str) -> AgentRunnable` — 持久 agent，可以跨多次调用反复用（异步上下文管理器）。
