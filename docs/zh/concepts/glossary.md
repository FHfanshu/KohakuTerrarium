...
所有外部信号统一进入系统时用的那层封装。用户输入、定时器触发、tool 完成、频道消息、sub-agent 输出，以及通过输出路由自动转发过来的 creature 输出，最后都会变成 `TriggerEvent(type=..., content=..., ...)`。入口只有一种，代码路径也只有一条。详见：[组合 agent](/concepts/foundations/composing-an-agent.md)（英文）。
...
一个专门负责把多个 creatures 接起来一起跑的层。它没有 LLM，也不做决策，就是一个运行时、一组共享 channels，再加上输出路由这套自动接线。creatures 不需要知道自己是不是在 terrarium 里；单独跑也照样成立。我们现在把它看成横向多智能体的一种候选架构，已经能用，但具体最佳实践还在继续长。进展和开放问题可以看 [roadmap](https://github.com/Kohaku-Lab/KohakuTerrarium/blob/main/ROADMAP.md)（英文）。详见：[terrarium](/concepts/multi-agent/terrarium.md)（英文）。
...
官方提供的开箱即用包，里面有一批实用的 creatures、terrariums 和 plugins，以 package 的形式发布。它不属于核心框架，更像展示样例和起步材料。见 [github.com/Kohaku-Lab/kt-biome](https://github.com/Kohaku-Lab/kt-biome)（英文）。
