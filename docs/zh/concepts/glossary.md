...
LLM 可以带参数调用的一项具名能力。比如 shell 命令、文件编辑、网页搜索。tool 也可以是消息总线、状态句柄，或者嵌套 agent——框架不限制一次调用背后具体发生什么。详见：[tool](/concepts/modules/tool.md)（英文）。

## Sub-agent

父级为了处理一个边界明确的任务而拉起的嵌套 creature。它有自己的上下文，通常也只拿到父级 tools 的一个子集。从概念上说，它也算一种 tool——站在 LLM 的视角，调用 sub-agent 和调用普通 tool 没区别。详见：[sub-agent](/concepts/modules/sub-agent.md)（英文）。
...
一个只负责连线的层，用来把多个 creatures 一起跑起来。它没有 LLM，也不做决策——就是一个运行时外加一组共享 channels。creatures 不知道自己身处 terrarium；单独运行时它们照样成立。这个部分现在边角还比较糙，可以看一下 [roadmap](https://github.com/Kohaku-Lab/KohakuTerrarium/blob/main/ROADMAP.md)（英文）。详见：[terrarium](/concepts/multi-agent/terrarium.md)（英文）。
...
- [概念索引](/concepts/README.md)（英文）——完整的章节地图。
