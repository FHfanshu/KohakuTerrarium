...
先用一句人话讲明白：这个仪表盘不是那种写死的固定排版，它本质上是一套“能切成两半、再继续切”的布局系统。屏幕上的每一块，要么就是一个真正显示内容的面板，要么就是一个“分割节点”，负责把空间继续拆成两块，中间那条分隔线还能直接拖。你切换 preset 的时候，不是小修小补，而是整套布局一下子换掉；进编辑模式的时候，则是在当前这套布局上原地改。
...
你在编辑模式里改的，不是那份“原件”，而是 preset 的一个**深拷贝**。所以只要你还没点保存，原来的布局其实一直都没动。这也是为什么你可以放心乱试，试完不满意再 Revert。
