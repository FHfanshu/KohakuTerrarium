...
很多人第一个装的 package 都是 `kt-biome`。这是官方示例包，里面带了 `swe`、`reviewer`、`researcher`、`ops`、`creative`、`general`、`root` 这些 creatures，也有 `swe_team`、`deep_research` 这样的 terrariums，还附带了一些 plugins。
...
如果你要做自己的 package，先拿 `kt-biome` 当参考最直接。
