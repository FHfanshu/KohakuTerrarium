...
大多数人第一个安装的包是 `kt-defaults` —— 这是一个示例包，包含 `swe`、`reviewer`、`researcher`、`ops`、`creative`、`general`、`root` 这些 creatures，还有 `swe_team` 和 `deep_research` 这些 terrariums，以及一些插件。
...
- [`kt-defaults`](https://github.com/Kohaku-Lab/kt-defaults) —— 参考包